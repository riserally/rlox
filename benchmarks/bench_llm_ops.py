...
def bench_rlox_grpo(n_prompts: int, k: int) -> BenchmarkResult:
...
def bench_numpy_grpo(n_prompts: int, k: int) -> BenchmarkResult:
    rng = np.random.default_rng(42)
    groups = [rng.standard_normal(k) for _ in range(n_prompts)]
...
def bench_torch_grpo(n_prompts: int, k: int) -> BenchmarkResult | None:
    try:
        import torch
    except ImportError:
        return None
...
def bench_rlox_kl(seq_len: int) -> BenchmarkResult:
...
def bench_numpy_kl(seq_len: int) -> BenchmarkResult:
    rng = np.random.default_rng(42)
    log_p = rng.standard_normal(seq_len)
    log_q = rng.standard_normal(seq_len)
...
def bench_torch_kl(seq_len: int) -> BenchmarkResult | None:
    try:
        import torch
    except ImportError:
        return None
...
def run_all(output_dir: str = "benchmark_results"):
    ...
    for n_prompts, k in [(16, 4), (64, 8), (256, 16)]:
        ...
        torch_res = bench_torch_grpo(n_prompts, k)
        if torch_res:
    ...
    for seq_len in [128, 512, 2048, 8192]:
        ...
        rlox_res = bench_rlox_kl(seq_len)
        print(f"    rlox:    {rlox_res.median_ns/1e3:>8.1f} us")
        all_results.append(rlox_res.summary())

        numpy_res = bench_numpy_kl(seq_len)
        print(f"    numpy:   {numpy_res.median_ns/1e3:>8.1f} us")
        all_results.append(numpy_res.summary())
        ...
        torch_res = bench_torch_kl(seq_len)
        if torch_res:
            print(f"    pytorch: {torch_res.median_ns/1e3:>8.1f} us")
            all_results.append(torch_res.summary())
