...
The **reward** $r_t \in \mathbb{R}$ is a scalar feedback signal the environment
returns after each action. It is the only training signal in RL -- there are no
labels.

- CartPole: $r_t = +1$ for every timestep the pole stays upright (maximum 500).
- Pendulum: $r_t = -(\theta^2 + 0.1 \dot{\theta}^2 + 0.001 u^2)$ (angle, velocity, torque cost).

The agent's objective is to maximise the **expected cumulative reward**:

$$J(\pi) = \mathbb{E}_\pi\left[\sum_{t=0}^{T} \gamma^t r_t\right]$$

This is what makes RL different from greedy optimisation -- the agent must
consider future consequences, not just immediate reward.
...
A **policy** $\pi_\theta(a \mid s)$ maps observations to actions. In deep RL,
the policy is a neural network parameterised by $\theta$.

- **Discrete** (e.g., CartPole): the network outputs a probability distribution
  over actions $\pi_\theta(a \mid s) = \text{softmax}(f_\theta(s))$
- **Continuous** (e.g., Pendulum): the network outputs the mean $\mu_\theta(s)$
  of a Gaussian: $\pi_\theta(a \mid s) = \mathcal{N}(\mu_\theta(s), \sigma^2)$
...
The **state-value function** $V^\pi(s)$ estimates the expected total future
reward starting from state $s$ and following policy $\pi$:

$$V^\pi(s) = \mathbb{E}_\pi\left[\sum_{t=0}^{\infty} \gamma^t r_t \mid s_0 = s\right]$$

It answers the question: "how good is it to be in this state?"
...
$\gamma \in [0, 1]$ (typically 0.99) controls this trade-off:

$$G_t = r_t + \gamma r_{t+1} + \gamma^2 r_{t+2} + \cdots = \sum_{k=0}^{\infty} \gamma^k r_{t+k}$$

- $\gamma = 0$: agent is completely myopic (only cares about immediate reward)
- $\gamma = 1$: agent weights all future rewards equally (can diverge)
- $\gamma = 0.99$: standard choice, balancing short and long-term
...
The relationship between value function and return is:
...
The agent learns **only from data collected by the current policy** $\pi_\theta$.
The policy gradient theorem gives the update direction:

$$\nabla_\theta J(\theta) = \mathbb{E}_{\pi_\theta}\left[\nabla_\theta \log \pi_\theta(a \mid s) \cdot \hat{A}(s, a)\right]$$

After each update, the old data is discarded because it was generated by a
...
The agent stores **all** experience in a **replay buffer** and learns by
minimising the **Bellman error** — the gap between the current Q-estimate
and the Bellman target:

$$\mathcal{L}(\phi) = \mathbb{E}_{(s,a,r,s') \sim \mathcal{D}}\left[\left(Q_\phi(s, a) - \underbrace{r + \gamma (1-d) \max_{a'} Q_{\bar\phi}(s', a')}_{\text{Bellman target}}\right)^2\right]$$

where $\mathcal{D}$ is the replay buffer, $d$ is the done flag, and
$\bar\phi$ are the target network parameters (slowly updated via Polyak
averaging: $\bar\phi \leftarrow \tau \phi + (1-\tau)\bar\phi$).
...
PPO ([Schulman et al., 2017](https://arxiv.org/abs/1707.06347)) is the
workhorse of on-policy RL. It prevents destructively large policy updates
by **clipping** the probability ratio between the new and old policy.
...
See [PPO algorithm page](algorithms/ppo.md) for full details.

### SAC (Soft Actor-Critic)

SAC ([Haarnoja et al., 2018](https://arxiv.org/abs/1801.01290)) is the go-to
off-policy algorithm for continuous control. It maximises reward **and** policy
entropy, encouraging exploration:

$$J(\pi) = \mathbb{E}_\pi\left[\sum_{t=0}^{T} \gamma^t \left(r_t + \alpha \mathcal{H}[\pi(\cdot|s_t)]\right)\right]$$

where $\alpha$ is the automatically-tuned temperature parameter.
...
See [SAC algorithm page](algorithms/sac.md) for full details.

### DQN (Deep Q-Network)

DQN ([Mnih et al., 2015](https://www.nature.com/articles/nature14236)) learns
a Q-function $Q_\phi(s, a)$ and takes the greedy action $a^* = \arg\max_a Q(s,a)$.
The Q-function is trained via the Bellman equation:

$$\mathcal{L}(\phi) = \mathbb{E}\left[\left(Q_\phi(s,a) - r - \gamma(1-d)\max_{a'} Q_{\bar\phi}(s',a')\right)^2\right]$$

rlox supports Rainbow extensions:
[Double DQN](https://arxiv.org/abs/1509.06461),
[Dueling](https://arxiv.org/abs/1511.06581),
[N-step returns](https://arxiv.org/abs/1710.02298), and
...
# Rainbow-style DQN
trainer = Trainer("dqn", env="CartPole-v1", seed=42, config={
    "double_dqn": True, "dueling": True, "n_step": 3, "prioritized": True,
})
metrics = trainer.train(total_timesteps=50_000)
```

See [DQN algorithm page](algorithms/dqn.md) for full details.

### TD3 (Twin Delayed DDPG)

TD3 ([Fujimoto et al., 2018](https://arxiv.org/abs/1802.09477)) improves
DDPG with three techniques: twin critics (take the minimum of two Q-values),
delayed policy updates, and target policy smoothing:

$$y = r + \gamma(1-d)\min_{i=1,2} Q_{\bar\phi_i}(s', \tilde{a}'), \quad \tilde{a}' = \pi_{\bar\theta}(s') + \text{clip}(\epsilon, -c, c)$$

```python
from rlox import Trainer

trainer = Trainer("td3", env="Pendulum-v1", seed=42, config={
    "policy_delay": 2, "target_noise": 0.2, "noise_clip": 0.5,
})
metrics = trainer.train(total_timesteps=20_000)
```

See [TD3 algorithm page](algorithms/td3.md) for full details.

### GRPO (Group Relative Policy Optimization)

GRPO ([Shao et al., 2024](https://arxiv.org/abs/2402.03300)) is designed for
LLM post-training. It generates **K completions** per prompt and normalises
rewards within each group:

$$\hat{A}_i = \frac{r_i - \bar{r}}{\sigma_r}, \quad \bar{r} = \frac{1}{K}\sum_{j=1}^{K} r_j$$

See [Section 9](#9-llm-post-training) for a full example and
[GRPO algorithm page](algorithms/grpo.md) for details.
...
The **advantage** $A^\pi(s, a)$ measures how much better action $a$ is
compared to the average action from state $s$:

$$A^\pi(s, a) = Q^\pi(s, a) - V^\pi(s)$$

- $A > 0$: the action was **better** than expected
- $A < 0$: the action was **worse** than expected

The core of policy gradient methods is simple: **increase the probability
of positive-advantage actions, decrease the probability of negative-advantage
actions.**

### The Bias-Variance Tradeoff

Computing advantages involves a fundamental tradeoff:

**Monte Carlo** (low bias, high variance):

$$\hat{A}_t^{MC} = G_t - V(s_t) = \left(\sum_{k=0}^{T-t} \gamma^k r_{t+k}\right) - V(s_t)$$

Unbiased but noisy — a single lucky trajectory inflates the estimate.
...
GAE ([Schulman et al., 2016](https://arxiv.org/abs/1506.02438)) interpolates
between these extremes using a parameter $\lambda \in [0, 1]$:
...
| $\lambda$ | Reduces to | Bias | Variance |
|-----------|-----------|------|----------|
| $0$ | TD residual $\delta_t$ | High | Low |
| $1$ | Monte Carlo $G_t - V(s_t)$ | Low | High |
| $0.95$ | Standard default | Balanced | Balanced |
