#!/bin/bash
set -euo pipefail
exec > /var/log/rlox-v8.log 2>&1

echo "=== v8: Targeted rlox PPO re-run (5 experiments) ==="
...
# Build Docker image
cat > /tmp/Dockerfile.v8 << 'DOCKERFILE'
...
docker build -t rlox-v8 -f /tmp/Dockerfile.v8 /tmp/rlox

echo "=== Running 5 rlox PPO experiments ==="
mkdir -p /tmp/rlox/results/convergence

docker run --rm -v /tmp/rlox/results:/app/results rlox-v8 python -c "
...
EXPERIMENTS = [
    ...
    ('HalfCheetah-v4', 1_000_000, {'n_envs': 8, 'n_steps': 2048, 'n_epochs': 10, 'batch_size': 64, 'learning_rate': 3e-4, 'normalize_obs': True, 'normalize_rewards': True, 'ent_coef': 0.0}),
    ('Hopper-v4', 1_000_000, {'n_envs': 8, 'n_steps': 2048, 'n_epochs': 10, 'batch_size': 64, 'learning_rate': 3e-4, 'normalize_obs': True, 'normalize_rewards': True, 'ent_coef': 0.0}),
    ('Walker2d-v4', 2_000_000, {'n_envs': 8, 'n_steps': 2048, 'n_epochs': 10, 'batch_size': 64, 'learning_rate': 3e-4, 'normalize_obs': True, 'normalize_rewards': True, 'ent_coef': 0.0}),
]
...
gcloud storage cp -r /tmp/rlox/results/convergence/ "gs://rkox-bench-results/convergence-v8-${TIMESTAMP}/" 2>&1 || true
gcloud storage cp /var/log/rlox-v8.log "gs://rkox-bench-results/convergence-v8-${TIMESTAMP}/" 2>&1 || true
